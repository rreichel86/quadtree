...
100   if(ior.lt.0) write(*,5000)
      ...
      ns     = td(3)
      ntyp   = td(8)
      if(ntyp.lt.10.or.ntyp.eq.16.or.ntyp.eq.592) then
c....   2-d generations
        nt     = 1
        ni     = td(4)
        ne     = td(5)
...
c.... determine last element number to be generated
      if(ntyp.lt.10.or.ntyp.eq.16.or.ntyp.eq.592) then ! 2D
        if (ntyp.eq.0) then
          nf = ne + nr*ns - 1
        else if (ntyp.eq.7) then ! 6–node triangles
          nf = ne + (nr*ns)/2 - 1
        else if (ntyp.eq.8.or.ntyp.eq.9.or.ntyp.eq.592) then ! 8/9–node quadrilaterals
          nf = ne + (nr*ns)/4 - 1
