...
c       Load case 9: Analytical traction for a infinite plate with a circular hole of 
c                    radius 'a' under uniaxial tension 'qx' in x= -\infty and x = \infty                                            
c-----------------------------------------------------------------------
      USE errchk
      USE iofile
      implicit double precision (a-h,o-z)
     ...
     +          dh2(3),dh3(3)
      ...
      real*8 :: x_pts(2,5), f9(2,5), ri, qx  ! for special load case 9
...
3002  format('Input:iload,icase,loads due to icase >',$)
      ...
      if(iload.eq.3) cload='   cubic   '
      if(iload.eq.4) cload='  quartic  '
      if(iload.eq.5) cload='  quintic  '
...
c.....copy td1->td to have old numbers
      ...
      icase = td(1)
c...  Special load case: Analytical traction for a infinite
c...  plate with a circular hole of radius 'a' under uniaxial tension 
c...  'qx' in x = -\infty and x = \infty
c...  ANALYTICAL TRACTION TO EQ. NODAL LOADS
      if(icase.eq.9) then
         ri = td(2)
        qx = td(3) 
        ...
        call eload2(ils)
c        
        if(prt)              write(iow,2099) ri, qx      
        if(prt.and.ior.lt.0) write(*  ,2099) ri, qx  
...
c....   loop over all elements
        if(iload.eq.1) then       ! linear
           ...
           inc = 2
        else if(iload.eq.3) then   ! cubic
           inc = 3
        else if(iload.eq.4) then   !quartic
           inc = 4
        end if
...
c....     load projection length
          ...
          call nodal_load(iload+1, x_pts, ri, qx, f9)
...
c....         interactive    (ior.lt.0)  id=-i/k via macro>mesh         
              if(ior.ge.0) then        
                if(id(j,n1).eq.0) f(j,n1) = f(j,n1) + f9(j,1)*dls
                if(id(j,n2).eq.0) f(j,n2) = f(j,n2) + f9(j,2)*dls
              else if(ior.lt.0) then          
                if(id(j,n1).gt.0) f(j,n1) = f(j,n1) + f9(j,1)*dls
                if(id(j,n2).gt.0) f(j,n2) = f(j,n2) + f9(j,2)*dls
              end if
              
              if(iload.eq.2) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                end if 
              end if
              ...
              if(iload.eq.3) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                  if(id(j,n4).eq.0) f(j,n4)=f(j,n4) + f9(j,4)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                  if(id(j,n4).gt.0) f(j,n4)=f(j,n4) + f9(j,4)*dls
                end if 
              end if
              ...
              if(iload.eq.4) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                  if(id(j,n4).eq.0) f(j,n4)=f(j,n4) + f9(j,4)*dls
                  if(id(j,n5).eq.0) f(j,n5)=f(j,n5) + f9(j,5)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + f9(j,3)*dls
                  if(id(j,n4).gt.0) f(j,n4)=f(j,n4) + f9(j,4)*dls
                  if(id(j,n5).gt.0) f(j,n5)=f(j,n5) + f9(j,5)*dls
                end if 
              end if
...
c....   loop over all elements
        if(iload.eq.1) then       ! linear
           ...
           c2 = 16.d0/45.d0
           c3 = 2.d0/15.d0
        else if(iload.eq.5) then   !quintic
           inc = 5
            c1 = 18.d0/288.d0
            c2 = 25.d0/96.d0      
            c3 = 25.d0/144.d0
...
c....         interactive    (ior.lt.0)  id=-i/k via macro>mesh         
              ...
              if(iload.eq.3) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + 3.d0*c1*q0(j)*dls
                  if(id(j,n4).eq.0) f(j,n4)=f(j,n4) + 3.d0*c1*q0(j)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + 3.d0*c1*q0(j)*dls
                  if(id(j,n4).gt.0) f(j,n4)=f(j,n4) + 3.d0*c1*q0(j)*dls
                end if 
              end if
              
              if(iload.eq.4) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + c2*q0(j)*dls
                  if(id(j,n4).eq.0) f(j,n4)=f(j,n4) + c3*q0(j)*dls
                  if(id(j,n5).eq.0) f(j,n5)=f(j,n5) + c2*q0(j)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + c2*q0(j)*dls
                  if(id(j,n4).gt.0) f(j,n4)=f(j,n4) + c3*q0(j)*dls
                  if(id(j,n5).gt.0) f(j,n5)=f(j,n5) + c2*q0(j)*dls
                end if 
              end if
              ...
              if(iload.eq.5) then
                if(ior.ge.0) then          
                  if(id(j,n3).eq.0) f(j,n3)=f(j,n3) + c2*q0(j)*dls
                  if(id(j,n4).eq.0) f(j,n4)=f(j,n4) + c3*q0(j)*dls
                  if(id(j,n5).eq.0) f(j,n5)=f(j,n5) + c3*q0(j)*dls
                  if(id(j,n6).eq.0) f(j,n6)=f(j,n6) + c2*q0(j)*dls
                else if(ior.lt.0) then         
                  if(id(j,n3).gt.0) f(j,n3)=f(j,n3) + c2*q0(j)*dls
                  if(id(j,n4).gt.0) f(j,n4)=f(j,n4) + c3*q0(j)*dls
                  if(id(j,n5).gt.0) f(j,n5)=f(j,n5) + c3*q0(j)*dls
                  if(id(j,n6).eq.0) f(j,n6)=f(j,n6) + c2*q0(j)*dls
                end if 
              end if
...
2099  format(' Load case 9: Analytical traction for a infinite plate',/,
     +       ' with a circular hole of radius = ',f12.5 ,/
     +       ' under uniaxial tension qx = ',f12.5 ,/
     +       ' in x = -infinify and x = infinity.')
