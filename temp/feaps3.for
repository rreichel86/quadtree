...
100   if(ior.lt.0) write(*,3001)
      ...
      za  = 0.d0
      ze  = 0.d0
cwd   if(ndm.eq.3) then IGA ndm=4 
      if(ndm.ge.3) then
         za  = td(5)
         ze  = td(6)
      end if
...
cwd   additional macro to determine number of rotational DOFs      
      if(pcomp(titl(1),'ro56',4)) go to 520
...
c
      call dinput(tl,9)
      numnp  =tl(1)   ! total number of nodal points in the problem
      numel  =tl(2)   ! total number of elements in the problem
      nummat =tl(3)   ! number of material property sets in the problem
      ndm    =tl(4)   ! number of spatial coordinates needed to define mesh
      ndf    =tl(5)   ! maximum number of degrees–of–freedom on any node
      nen    =tl(6)   ! maximum number of nodes on any element
      nad    =tl(7)   ! increases size of element arrays to ndf*nen + nad
      ndd    =tl(8)   ! maximum number of parameters for element properties
      nthread=tl(9)   !
      nde = 0
      ned = 0
      ndd = max(ndd,50)
...
2015    format(/5x,'Tie all nodes with common coordinates'/1x)
     ...
     1         ndm,ndf,nen,nen1,numnp,numel,ntied,l1,l2,l3,l4,j,td,prt)
      ...
      go to 1
cwd.. Determine number of rotational DOFs for every node (control point) and set
c     bounday condition of DOF 6 accordingly      
c...  [ro56]
c...  [ro56,angl,tol]
c...  [ro56,<angl>,<tol>,overwriteall]  
c...  [ro56,<angl>,<tol>,<overwriteall>,useglobalcart]        
c     tol is the angle in degrees between the directors which seperates kinks from smooth transitions
c...  if not specified with angle, the tolerance is 0.5 degrees      
c...  overwriteall specifies if all 6th DOF BC shall be set(value=1), or only for those on edges
c...  id(6,node)=0 shall be set(value=0).   
c...  useglobalcart specifies if for 6 DOF nodes a global cartesian coordinate system shall bes used
c...  useglobalcart=1: (1 0 0),(0 1 0), (0 0 1) as nodal coordinate system, 0: do not change anything      
520   continue      
c     check if global number of DOFs is larger than 5
      if (ndf.le.5) then
        write (iow,2017) ndf
        write (*  ,2017) ndf
c        go to 1 
      ...
      call acheck(titl,yyy,15,80,80)
      read(yyy,1002,err=600,end=700) titl(1),titl(2),(td(i),i=1,3)
c     get tolerance angle
      if(pcomp(titl(2),'angl',4)) then
        tol = td(1)
      else
        tol = 0.5d0
      end if
      ioverwriteall = td(2)
      iuseglobalcart = td(3)
      if (iuseglobalcart.ne.0.and.iuseglobalcart.ne.1) iuseglobalcart=0
2017  format ('ndm =  ',i1,': SET NUMBER OF GLOBAL DOFs (ndm) > 5 TO USE
     + THE "ro56" COMMAND')  
      if (knode.ne.numnp) then
        write(iow,2018)
        write(*  ,2018)
c2018    format ('ERROR OCCURED IN DEFINITION OF BASIS SYSTEMS')  
      end if
c     call routine to set boundary conditions    
csk      call setdrilldof(m(n7),m(n9),m(nip),ndm,ndf,nen,nen1,numnp,numel,
csk     1                 tol,m(mdir),prt,ioverwriteall,iuseglobalcart,ipr)
csk      call setdrilldof(psid,econ,gtie,ndm,ndf,nen,nen1,numnp,numel,
csk     1                 tol,basea,prt,ioverwriteall,iuseglobalcart,ipr)
      call setdrilldof(psid,gtie,ndf,numnp,
     1                 tol,basea,prt,ioverwriteall,iuseglobalcart)
c     (id,ix,ip,...)
      go to 1
cwd   end of function      
...
c-----------------------------------------------------------------
      ...
      fa=.false.
      idtyp =xdir(1)
cwd   for NURBS shells director is needed earlier->leave this function
c     copy dir(:,:,1) to dir(:,:,2)
      if (idtyp.eq.20.or.idtyp.eq.21) then
        do i = 1,numnp
           do k = 1,9
              dir(k,i,2) = dir(k,i,1)
            end do
...
cwd   Director for NURBS shells
c      if (idtyp.ge.18.and.idtyp.le.20) then
c        if(idtyp.eq.18) head1=' NURBS control point net lin. int.'
c        if(idtyp.eq.19) head1=' NURBS control point net lin. int2.'
c        if(idtyp.eq.20) head1=' NURBS derivatives'
c        if (idtyp.eq.18.or.idtyp.eq.19)
c     +    call pdireciga1(idtyp,x,dir,numnp,ndm,1)
c        if (idtyp.eq.20) call pdireciga2(idtyp,x,dir,numnp,ndm,1,gtie)
c        do i = 1,numnp
c          node = dir(10,i,1)
c          if(node.gt.0) then
c            do k = 1,9
c              dir(k,i,1) = dir(k,i,1) / node
c            end do
c            call plamina_base(dir(1,i,1))
c          end if
c        end do
c      end if
cwd
...
c     14=str1: wie 08
     ...
     +   .or.isw.eq.13 .or. isw.eq.14 .or.nn1.eq.nn2 .or.isw.eq.23) then
        parforms = parform ! save
        parform=.false.
      end if     
cwd	  save old ix list for IGA
csk     if (isNurbs()) call getoldix(m(Inoix),oldix,numel,nen)
      if (isNurbs()) call getoldix(AInoix,oldix,numel,nen)
...
!$OMP&    afl,bfl,istyp,japt,lodrv,cfl,hflgu,h3flgu,oldix,
...
c....  local temp, coord,displ,veloc,accel on element nodes
        do 108 i = 1,nen
          ...
          if(ii.gt.0) then
            ...
            do j = 1,ndm
              xl(j,i) = x(j,ii)
cwd			  get weighting of original control point in IGA			  
              if (isNurbs().and.j.eq.4) xl(j,i) = x(j,oldix(i,n))
            end do  
            do 107 j = 1,ndf
              jj = ie(j,ma)
              if(jj.gt.0) then
...
!$OMP END PARALLEL
      ...
      if (isw.eq. 2 .or. isw.eq. 4 .or.isw.eq. 8 .or.isw.eq. 9
     +.or.isw.eq.13 .or. isw.eq.14 .or.nn1.eq.nn2 
     +.or.isw.eq.23) parform = parforms ! reset
